...
7. **This repository is public. Everything committed is permanent and world-readable within minutes.**
   ...
   **Nothing enters a tracked file, a commit message, or a build artefact except a description of the tool and its behaviour.** That is the whole of the permitted content. If a sentence explains what the tool does, why it refuses what it refuses, or how a claim about it was measured, it may be committed. If it explains anything else, it may not.
   ...
   2. **Never reproduce what was caught — in gate output, in a commit message, or in a document.** Any check for secrets or identifiers redacts before output. CI logs on a public repository are public, so a scanner that echoes its match converts a contained mistake into a disclosed one. Report filenames and counts, never content.

      **The rule covers the writing about the leak, not only the tooling.** A commit message describing a fix must not contain the value it fixed; nor must a doc explaining the incident, or a test name, or a code comment. This is not hypothetical: the commit that added the UTC check states the offending offset in plain text, in a public message, and it was caught only by a later audit. The value was already public in the offending commit's metadata, so nothing new was disclosed — but it went from something you had to inspect timestamps to notice, into readable prose.

      The pull is strong and worth naming: a good commit message explains *what was wrong*, and the most concrete way to do that is to quote it. Write the shape instead. "A commit arrived with a non-UTC offset" carries the same meaning as naming the offset, and a reader who needs the value can find it where it already is. **The same sentence that documents a fix should not be the most convenient place to find what it fixed.**
